//! 角色包清单（manifest.json）类型定义、读写校验，以及 markdown/lore 分块与文本纯函数。
//!
//! 本文件只承载与 manifest 直接相关的逻辑：
//! - 所有 `pub` 数据类型（PackManifest / CharacterCard / SpeechStyle ...）与 lorebook 召回面板用的公开 DTO。
//! - lorebook 内部共享类型（LoreChunk / LoreHit / LoreIndexCache ...）以 `pub(super)` 暴露给兄弟模块。
//! - manifest 读写、路径校验、avatar data url、zip 导入解压、persona 渲染。
//! - 纯文本/分块辅助函数（query_terms / normalize_search_text / dedupe_strings / split_lore_markdown ...）。
//!
//! 跨子模块引用：`use super::manifest::{LoreChunk, ...}`。
...
pub(super) const MAX_IMPORT_BYTES: u64 = 25 * 1024 * 1024;
pub(super) const LORE_INDEX_VERSION: u32 = 1;
pub(super) const MAX_LORE_FILE_BYTES: u64 = 256 * 1024;
pub(super) const MAX_LORE_INDEX_FILES: usize = 500;
pub(super) const MAX_LORE_CHUNK_CHARS: usize = 1_200;
pub(super) const MAX_LORE_CONTEXT_CHARS: usize = 4_000;
pub(super) const MAX_LORE_CONTEXT_CHUNKS: usize = 4;
pub(super) const DEFAULT_LORE_EXTENSIONS: &[&str] = &["md", "markdown", "txt"];
pub(super) const MAX_PACK_READ_BYTES: u64 = 512 * 1024;
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub(super) struct LoreFileSignature {
...
#[derive(Serialize, Deserialize, Clone, Debug)]
pub(super) struct LoreIndexCache {
    pub(super) version: u32,
    pub(super) pack_id: String,
    pub(super) files: Vec<LoreFileSignature>,
    pub(super) chunks: Vec<LoreChunk>,
...
#[derive(Clone, Debug)]
pub(super) struct LoreSource {
    pub(super) rel_path: String,
    pub(super) title: Option<String>,
    pub(super) tags: Vec<String>,
    pub(super) priority: Option<f32>,
}

#[derive(Clone, Debug)]
pub(super) struct MarkdownMeta {
    pub(super) title: Option<String>,
    pub(super) tags: Vec<String>,
    pub(super) keywords: Vec<String>,
    pub(super) priority: Option<f32>,
...
#[derive(Clone, Debug)]
pub(super) struct LoreHit {
    pub(super) score: f32,
    pub(super) chunk: LoreChunk,
...
pub(super) fn read_manifest_with_avatar(dir: &Path) -> Result<PackManifest, String> {
...
pub(super) fn read_manifest_no_avatar(dir: &Path) -> Result<PackManifest, String> {
...
pub(super) fn validate_manifest_paths(manifest: &PackManifest) -> Result<(), String> {
...
pub(super) fn validate_lore_file_extension(path: &str, lore: &LoreEntry) -> Result<(), String> {
...
pub(super) fn validate_relative_file(value: &str, field: &str) -> Result<(), String> {
...
pub(super) fn resolve_pack_file(dir: &Path, rel: &str, field: &str) -> Result<PathBuf, String> {
...
pub(super) fn avatar_mime(path: &str) -> Option<&'static str> {
    match Path::new(path)
        .extension()
        .and_then(|v| v.to_str())
...
pub(super) fn validate_pack_files(dir: &Path, manifest: &PackManifest) -> Result<(), String> {
    let persona_path = resolve_pack_file(dir, &manifest.persona, "persona")?;
...
pub(super) fn lore_file_signatures(
    dir: &Path,
...
pub(super) fn parse_markdown_meta(text: &str) -> MarkdownMeta {
...
pub(super) fn split_lore_markdown(
...
pub(super) fn split_by_chars(text: &str, max_chars: usize) -> Vec<String> {
...
pub(super) fn merge_unique(first: Vec<String>, second: Vec<String>) -> Vec<String> {

...
- Voice：语音输入（STT/ASR）已接入云端转写后端（DashScope `qwen3-asr-flash`、OpenAI 兼容 Whisper，由 `voice_stt_backend` 选择）；语音输出（TTS）接口仍为预留占位，后续可接 GPT-SoVITS、CosyVoice 等。
...
│  ├─ src/voice.rs               # Voice adapters (STT wired, TTS reserved)

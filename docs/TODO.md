...
- [x] **搜索导航工具**：`glob`、`grep`、`git_status`。
- [x] **Shell 工具**：确认门、沙盒 cwd、超时、输出截断、执行前预览。
- [x] **Web Search**：Bing、DuckDuckGo fallback、Tavily、Brave、Exa adapter，域名过滤、结果数量、context cap、Sources 提醒。
...
- [x] **API Key 安全存储**：LLM key 使用系统凭据管理器。
...
- [ ] **Web Search 设置 UI**：选择 adapter，配置 Tavily/Brave/Exa key，并接入 keyring。
...
- [ ] **子 Agent 证据包**：让子 Agent 输出结构化 findings/evidence/questions，便于主 Agent 合成。

## 中期方向

- [ ] **自定义 Agent 模板**：`.demiurge/agents/*.json`，定义 prompt、allowed tools、budget、handoff format。
- [ ] **per-agent 硬预算**：基于精确 provider usage 管理子 Agent 和 workflow step。
- [ ] **计划模式**：先生成计划，用户批准后再执行写入和 shell 类操作。
- [ ] **MCP 接入**：tool discovery、resource 读取、权限分级和设置 UI。
- [ ] **WebFetch / livecrawl**：把搜索结果深抓取、正文抽取和来源整理统一起来。

...
- **Workflow Journal / Worktree**：`/ultracode` run 会写 `.demiurge/workflow-runs/<run_id>/journal.jsonl`，`/workflows` 列 run，`/workflow resume <run_id>` 用 journal 生成恢复 overlay；`worktree_create` 可在沙盒 Git 仓库下创建隔离 worktree。

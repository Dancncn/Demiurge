...
| `llm/mod.rs` | OpenAI 兼容流式客户端，SSE 解析 + 流式 tool_calls 累积 | `stream_completion()` |
| `tools/mod.rs` | 工具注册表 + 统一执行入口 + 沙盒路径解析 | `registry()` / `execute()` |
| `tools/*.rs` | 各内置工具实现 | `open_path` / `read_file` / `write_file` / `web_search` / `system_info` |
...
| `tool-start` | `{name, args}` | 工具开始执行 |
| `tool-end` | `{name, ok, result}` | 工具结束（ok=是否放行） |
| `tool-confirm-request` | `{id, tool, args}` | 请前端弹确认框；前端回 `respond_confirm(id, allow)` |
...
**加一个工具**：在 `tools/mod.rs` 的 `registry()` 加一项（名称/描述/权限/JSON Schema），在 `execute()` 加一条分支，新建 `tools/<name>.rs` 实现。confirm 类会自动走确认门。

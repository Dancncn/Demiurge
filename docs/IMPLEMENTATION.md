...
| `agent/context.rs` | 对话历史裁剪（先截老工具输出，再丢更老回合） | `trim()` |
| `agent/prompt.rs` | Phase 2 prompt section builder：engine/persona/project/environment/memory 分区 | `build()` |
...
| `tools/*.rs` | 各内置工具实现 | `open_path` / `read_file` / `write_file` / `edit_file` / `shell` / `web_search` / `system_info` / `glob` / `grep` / `git_status` |
...
### Agent 循环（`run_turn`）
1. 追加用户消息、持久化。
2. 通过 `agent/prompt.rs` 拼分区化 system prompt（引擎规则、角色人格、项目指令、运行环境、只读 memory），再追加裁剪后的历史并流式调 LLM；正文增量通过 `assistant-delta` 事件实时推给前端。
...
这些 project/memory/environment section 总体再做字符上限截断，避免挤占对话历史预算。`agent/context.rs` 仍只负责历史消息裁剪；session rolling summary、token-aware budget 与自动记忆提取留作 Phase 2 后续增强。
...
前端命令（`invoke`）：`send` / `interrupt` / `respond_confirm(id, allow, scope)` / `get_settings` / `save_settings` / `list_packs` / `list_sessions` / `get_history` / `new_session` / `select_session` / `delete_session` / `open_sandbox`。
...
- **文件工具**：`read_file` / `write_file` / `edit_file` 限定在沙盒目录；`edit_file` 只修改已有 UTF-8 文本文件，默认要求 `old_string` 唯一，执行前在确认弹窗展示 diff preview。
...
- **项目上下文**：`DEMIURGE.md` / `CLAUDE.md` / memory 文件只从沙盒或当前角色包目录只读加载，单文件大小受限；读取失败不会中断 Agent 回合。

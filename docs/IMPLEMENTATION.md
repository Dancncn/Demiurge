...
| `tools/*.rs` | 各内置工具实现 | `open_path` / `read_file` / `write_file` / `edit_file` / `undo_edit` / `shell` / `web_search` / `system_info` / `glob` / `grep` / `git_status` |
...
- **文件工具**：`read_file` / `write_file` / `edit_file` / `undo_edit` 限定在沙盒目录；`edit_file` 只修改已有 UTF-8 文本文件，默认要求 `old_string` 唯一，执行前在确认弹窗展示 diff preview，成功后把编辑前后内容记录到进程内 undo 栈；`undo_edit` 只能撤销最近一次 `edit_file`，执行前同样确认并校验目标文件当前内容未偏离记录。

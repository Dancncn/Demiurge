...
```text
React UI
  ├─ invoke: send / settings / sessions / workflow / memory / permission / plan / WebDAV / OCR / voice commands
  └─ listen: assistant/tool/confirm/goal/workflow/plan events
        │
        ▼
Rust AppState
  ├─ agent runner
...
| `credentials.rs` | keyring 凭据读写，避免 LLM/Web Search/WebDAV 密钥落入 settings 明文 | `load_api_key()` / `save_api_key()` |
...
| `agent/budget.rs` | 启发式 token 预算、provider usage 汇总、history budget | `history_budget()` / `TokenBudgetState` |
...
| 模块 | 职责 |
|---|---|
| `src/App.tsx` | 主状态编排，订阅后端事件，维护消息流、设置、会话、Agent 选择、Plan Mode 控制和 workflow panel |
...
| `components/SettingsDialog.tsx` | provider、Web Search、OCR、语音、WebDAV、权限、记忆审计设置 |
...
API Key 和 WebDAV 密码存在系统凭据管理器中，不写入 `settings.json`。兼容迁移会读取旧 settings 明文字段并转存到 keyring。
...
3. 普通回合调用 `run_turn_with_options`，把用户消息写入 session；`send_with_agents` 会把前端选中的自定义 Agent 合并成 prompt overlay、工具限制和预算限制。

...
```text
React UI
  ├─ invoke: send / settings / sessions / workflow / memory / permission / WebDAV / OCR / voice commands
  └─ listen: assistant/tool/confirm/goal/workflow events
        │
        ▼
Rust AppState
  ├─ agent runner
...
| 模块 | 职责 |
|---|---|
| `src/App.tsx` | 主状态编排，订阅后端事件，维护消息流、设置、会话、Agent 选择和 workflow panel |

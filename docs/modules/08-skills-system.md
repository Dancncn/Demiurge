...
- **就地发现**：覆盖五类作用域目录，从全局应用数据到仓库内、再到对外部 CLI 同名约定的兼容目录（`.claude/`）。
...
`SkillScope`（`skills.rs:31-53`）是六值枚举：`Global / Project / Repository / Pack / Claude / Legacy`，其 `label()` 返回小写串（`global`/`project`/…），既用于面板序列化（`#[serde(rename_all = "snake_case")]`），也用于排序与拼 `id`。
...
| 5 | Claude | `{sandbox}/.claude/skills/` | 对外部 CLI 同名约定（`.claude/` 目录）的兼容入口 |

`sandbox` / `data_dir` / `packs_dir` 三个根来自 `AppState`（`lib.rs:58-60` 的 `data_dir` / `sandbox_dir` / `packs_dir`），`pack_id` 取自 `settings.current_pack`（`lib.rs:191` 等处）。`.claude/skills/` 仅是对其他工具同名目录布局的兼容读取，引擎不对该来源做任何特殊优待。

> **重要：发现顺序不等于优先级。** 上面的 `for` 顺序只决定「先把谁 push 进 catalog」。`discover` 末尾（`skills.rs:220-221`）会用 `(scope.label(), name)` 重新整体排序，所以最终列表的物理顺序是按 scope 标签字母序（`claude < global < legacy < pack < project < repository`）再按名字排。真正影响「哪条被注入」的是后续 `select` 的打分，而非发现顺序。同名 Skill 也**不会**互相覆盖——不同 scope 的同名 Skill 会因 `id = "{scope}-{name}"` 不同而共存。
...
- **Legacy 单文件来源固定**：`.demiurge/skills.md`、`skills/README.md`、`.claude/skills.md` 三条路径写死，且强制 `always_include`。

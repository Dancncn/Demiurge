...
代码中文件类工具被限制在沙盒目录，计划文件写入 `sandbox/.demiurge/plans/`（`write_plan.rs:17`）。`.demiurge/` 是本项目自有的沙盒内部目录约定。本子系统未直接引用 `CLAUDE_CODE_*` 环境变量或 `.claude/` 兼容目录；如在其他模块出现此类标识符，应理解为对外部 CLI 同名约定的兼容，而非品牌衍生关系。

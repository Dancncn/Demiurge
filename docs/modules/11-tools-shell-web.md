...
**Tavily**：endpoint 取 `env_first(["TAVILY_SEARCH_URL","TAVILY_ENDPOINT_URL"])`，默认值为 `https://api.tavily.com/search`（`web_search.rs:188`）。请求体含 `query/search_depth/max_results`，allowed/blocked 转成 `include_domains`/`exclude_domains`。key 经 `bearer_auth` + `x-api-key` 双注入。
